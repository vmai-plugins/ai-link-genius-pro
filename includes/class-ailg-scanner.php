...
class AILG_Scanner {
    ...
    public static function ajax_scan_single_item(): void {
        ...
        self::scan_post( $post_id, $target_id );

        wp_send_json_success();
    }

    public static function scan_post( int $post_id, int $target_id = 0 ): void {
        global $wpdb;
        $extra_args  = $target_id ? [ 'target_id' => $target_id ] : [];
        $suggestions = AILG_AI::generate_suggestions( $post_id, '', '', $extra_args );
        foreach ( $suggestions as $s ) {
            // Avoid duplicates in suggestions table
            $exists = $wpdb->get_var( $wpdb->prepare(
